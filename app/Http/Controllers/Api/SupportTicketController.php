...
class SupportTicketController extends Controller
{
    ...
    /**
     * عرض تذكرة محددة تخص العميل المسجل.
     */
    ...
    /**
     * إرسال رد من حساب العميل المسجل.
     */
    public function replyAuthenticated(
        Request $request,
        string $ticketNumber
    ): JsonResponse {
        $validated = $request->validate([
    ...
    private function ticketData(
        ...
        $hasAdminMessage = $messages->contains(
            fn (array $message): bool =>
                ($message['sender_type'] ?? null) === 'admin'
        );

        // الرد المحفوظ من لوحة Filament يوجد في admin_reply.
        // نضيفه للمحادثة إذا لم تكن هناك رسالة إدارة مسجلة أصلًا.
        if (filled($ticket->admin_reply) && ! $hasAdminMessage) {
            $messages->push([
                'id' =>
                    'admin-reply-' . $ticket->id,
                ...
                'message' =>
                    trim((string) $ticket->admin_reply),
                ...
                'created_at' =>
                    ($ticket->replied_at ?? $ticket->updated_at)
                        ?->toIso8601String(),
            ]);
        }
